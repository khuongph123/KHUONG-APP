...
app.get('/api/news', async (req, res) => {
    ...
    try {
        ...
        if (!BROWSERLESS_CONNECTION_STRING || BROWSERLESS_CONNECTION_STRING.includes('2TCK6pkKFF5RpZzb18f9a7c0de4f16f31a01ee454a87e10bb')) {
                ...
                if (titleEl && titleEl.href) {
                    ...
                    if (!articles.some(a => a.link === link)) {
                        articles.push({
                            ...
                            link: link,
                            description: el.querySelector('.td-excerpt, .entry-content')?.innerText.trim() || '',
                            imageUrl: el.querySelector('img.entry-thumb, .td-module-thumb img')?.src || null
                        });
                    }
                }
            });
        ...
        if (browser) {
            ...
            console.log("[LOG] Đã ngắt kết nối.");
        }
    }
});
